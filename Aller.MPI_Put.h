...
struct Aller {
    ...
    size_(0)
  {
    ...
    MPI_Win_create(recv,bytes,sizeof(*recv),MPI_INFO_NULL,comm_,&win_);
  ...
  {
    return __FILE__ ": MPI_Put (rank+1:)";
  }
  ...
  void run(const int count)
  {
    ...
    for (int i = 0; i < size_; i++) {
      ...
      const long *const addr = send_+(long(target)*long(count));
      MPI_Put(addr,count,MPI_LONG,target,disp,count,MPI_LONG,win_);
    }
    MPI_Win_fence(0,win_);
  }
};
