...
struct Aller {
    ...
    size_(0)
  {
    ...
    sreqs_.resize(size_,MPI_REQUEST_NULL);
  ...
  {
    return __FILE__ ": MPI_Irecv (rank-1::-1), MPI_Barrier, MPI_Isend (rank+1:)";
  }
  ...
  void run(const int count)
  {
    ...
    for (int i = 0; i < size_; i++) {
      ...
      MPI_Isend(buf,count,MPI_LONG,dest,tag_,comm_,&sreqs_[i]);
    }
    MPI_Waitall(size_,rreqs_.data(),MPI_STATUSES_IGNORE);
    MPI_Waitall(size_,sreqs_.data(),MPI_STATUSES_IGNORE);
  }
};
